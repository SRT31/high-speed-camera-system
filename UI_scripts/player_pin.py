...
# ------------------------- User Settings -------------------------
FRAMES_DIR     = "./Programs/player_keyboard/temp_frames"   # folder containing frames
...
JUMP_AMOUNT    = 5                 # [frames] per jump on J/K and Jump button
INTERP_FACTOR  = 4                 # 1=no interpolation, 2=+1 in-between frame, 4=+3, etc.
FARNE_PARAMS   = dict(pyr_scale=0.5, levels=4, winsize=25, iterations=5,
                      poly_n=7, poly_sigma=1.5, flags=0)
# -----------------------------------------------------------------

# GPIO button pins
BTN_STARTSTOP = 27 # Square button GPIO pin for Start/Stop button
BTN_REVERSE   = 23 # White Circle button GPIO pin for Reverse button
BTN_JUMP      = 24  # Blue Circle button GPIO pin for Jump button 

# Keyboard map (kept for convenience)
KEY_SPACE   = 32
KEY_ESC     = 27
KEYS_QUIT   = {ord('q'), ord('Q'), KEY_ESC}
KEYS_REV    = {ord('r'), ord('R')}
KEYS_STEP_B = {ord('a'), ord('A')}  # still supported via keyboard
...
request_step  = 0      # -1 / +1   (still supported from keyboard A/D)
...
def print_help():
  ...
  H     : Help
  Q / Esc : Quit
  GPIO27 : Start/Stop (toggle)
  GPIO23 : Reverse direction
  GPIO24 : Jump by ±JUMP_AMOUNT (respects current mode: keeps playing if playing; stays stopped if stopped)
""")
...
def btn_reverse(channel):
    global direction
    direction *= -1

def btn_jump(channel):
    # Add or subtract JUMP_AMOUNT based on current direction.
    # IMPORTANT: Do NOT change 'playing' here.
    global request_jump, direction
    request_jump += (JUMP_AMOUNT if direction > 0 else -JUMP_AMOUNT)
# -----------------------------------
...
def main():
    ...
    GPIO.setup(BTN_STARTSTOP, GPIO.IN, pull_up_down=GPIO.PUD_UP)
    GPIO.setup(BTN_REVERSE,   GPIO.IN, pull_up_down=GPIO.PUD_UP)
    GPIO.setup(BTN_JUMP,      GPIO.IN, pull_up_down=GPIO.PUD_UP)

    GPIO.add_event_detect(BTN_STARTSTOP, GPIO.FALLING, callback=btn_startstop, bouncetime=200)
    GPIO.add_event_detect(BTN_REVERSE,   GPIO.FALLING, callback=btn_reverse,   bouncetime=200)
    GPIO.add_event_detect(BTN_JUMP,      GPIO.FALLING, callback=btn_jump,      bouncetime=200)
    # -------------------
    ...
    if INTERP_FACTOR > 1:
        ...
        print(f"Interp done. Total frames: {len(frames)} (took {time.time()-t0:.2f} [s])")
    else:
